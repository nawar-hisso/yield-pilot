...
- **mUSDC faucet** — /vault → Faucet button → 1,000 mUSDC minted to EOA
...
- **Deposit — first time** — 2 Touch IDs (approve + deposit), ~60s total, funds in vault
...
- **Hero copy** — disconnected shows "Deposit. Earn." with Connect CTA
...
- **Command palette (⌘K)** — opens, navigates to Vault / Faucet / Settings
...
- **Passkey withdraw** — mirror `useDeposit`'s passkey branch. File: `apps/web/hooks/useWithdraw.ts`
- **Atomic passkey deposit** — Paymaster now accepts `executeBatch`; first deposit = 1 Touch ID
...
- **Etherscan verify** — retry `hardhat verify` on all 6 contracts once DNS resolves `api.etherscan.io`. 10 min
